
using Sand.Domain.Queries.Consults;
using Sand.Service.Dtos.Consults;
using Sand.Service.Contract.Consults;
using Microsoft.AspNetCore.Mvc;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using Sand.Result;
using Microsoft.AspNetCore.Cors;

namespace Sand.Api.Controllers.Consults {
    /// <summary>
    /// Consults-选项表服务
    /// </summary>
    [Route(ApiVersion)]
    [EnableCors("any")]
    public class OptionsController : BaseApiController {
         /// <summary>
        /// 选项表服务
        /// </summary>
        private readonly IOptionsService _optionsService;
        
        /// <summary>
        /// 初始化选项表控制器
        /// </summary>
        /// <param name="service">选项表服务</param>
        public OptionsController( IOptionsService service ){
            _optionsService = service;
        }
        
        /// <summary>
        /// 查询选项表信息
        /// </summary>
        /// <param name="optionsQuery">选项表查询对象</param>
        /// <returns>查询的成功信息（1:成功,2:失败）</returns>
        [HttpGet]
        public async Task<IActionResult> Get(OptionsQuery optionsQuery)
        {
            return Success(await  _optionsService.RetrieveAsync(optionsQuery));
        }
        
        /// <summary>
        /// 根据编号查询选项表信息
        /// </summary>
        /// <param name="id">编号</param>
        /// <returns>查询的成功信息（1:成功,2:失败）</returns>
        [HttpGet("{id}")]
        public async Task<IActionResult> Get(string id)
        {
            return Success(await _optionsService.RetrieveByIdAsync(id));
        }
        
        /// <summary>
        /// 请求选项表分页信息
        /// </summary>
        /// <param name="optionsQuery">选项表查询对象</param>
        /// <returns>查询的成功信息（1:成功,2:失败）</returns>
        [HttpGet("page", Name = "pageoptions")]
        public async Task<IActionResult> Page(OptionsQuery optionsQuery)
        {
            return Success(await _optionsService.PageAsync(optionsQuery));
        }
        
        /// <summary>
        /// 更新选项表信息
        /// </summary>
        /// <param name="options">选项表更新对象</param>
        /// <returns>更新结果信息（1:成功,2:失败）</returns>
        [HttpPost]
        public async  Task<IActionResult> Post([FromBody]OptionsDto options)
        {
           await _optionsService.UpdateAsync(options);
            return Success();
        }
        
         /// <summary>
        /// 新增选项表信息
        /// </summary>
        /// <param name="options">选项表信息</param>
        /// <returns>新增结果信息（1:成功,2:失败）</returns>
        [HttpPut]
        public async Task<IActionResult> Put([FromBody]OptionsDto options)
        {
            return Success(await _optionsService.CreateAsync(options));
        }
        
        /// <summary>
        /// 删除选项表信息
        /// </summary>
        /// <param name="options">选项表信息</param>
        /// <returns>删除结果信息（1:成功,2:失败）</returns>
        [HttpDelete]
        public async Task<IActionResult> Delete(List<OptionsDto> options)
        {
            await _optionsService.DeleteAsync(options);
            return Success();
        }
        
        /// <summary>
        /// 停用选项表信息
        /// </summary>
        /// <param name="options">选项表信息</param>
        /// <param name="isEnable">启用或者停用</param>
        /// <returns>停用或者启用结果信息（1:成功,2:失败）</returns>
        [HttpPost("stop", Name = "stopoptions")]
        public async Task<IActionResult> Stop(List<OptionsDto> options, bool isEnable=false)
        {
            await _optionsService.StopOrEnableAsync(options, isEnable);
            return Success();
        }
    }
}