...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="answerData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         <el-table-column prop="optionsId" label="选项值" width="100"></el-table-column>
         <el-table-column prop="stroeId" label="门店号" width="100"></el-table-column>
         <el-table-column prop="content" label="答案内容" width="100"></el-table-column>
         <el-table-column prop="type" label="类型 （回答类，问卷类）" width="100"></el-table-column>
         <el-table-column prop="value" label="选项值" width="100"></el-table-column>
         <el-table-column prop="suggestion" label="父级" width="100"></el-table-column>
 ...
 <answer-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></answer-edit>
</div>
</template>
<script>
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.answerData = respose.data.result
     ...
     */
    del(row) {
      const answer = this.selectdata(row)
      if (!answer || answer.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('answer', { answer }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Answer
    ...
    stop(row, isEnable) {
      ...
      if (answer.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Answer
     * @param 当前选中行
     */
