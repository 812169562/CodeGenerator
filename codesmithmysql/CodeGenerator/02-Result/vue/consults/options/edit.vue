<template>
 <el-dialog :title="title" :visible.sync="dialogVisible" width="55%" height="75%" :close-on-click-modal="false" :before-close="hidden" @open="open"  @close="hidden">
    <el-form :model="form">
         ...
         <mu-text-field label="问卷名称" hintText="请输入问卷名称" v-model="form.name"  lab labelFloat/>
  </el-form>
...
export default {
  ...
  methods: {
    ...
    save() {
      const _this = this
      this.$request.save('options', this.form, function(respose) {
        if (respose.code === 1) {
          _this.init()
          _this.hidden(true)
        }
    ...
    open() {
      if (!this.id) {
        ...
        this.title = '编辑选项表信息'
        const _this = this
        this.$request.get('options/' + this.id, {}, (respose) => {
