...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="questionnaireData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="context" label="内容" width="100"></el-table-column>
         <el-table-column prop="type" label="回答类，问卷类）" width="100"></el-table-column>
 ...
 <questionnaire-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></questionnaire-edit>
</div>
</template>
<script>
import QuestionnaireEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.questionnaireData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Questionnaire */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Questionnaire
     * @param 当前选中数据
     */
    del(row) {
      const questionnaire = this.selectdata(row)
      if (!questionnaire || questionnaire.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('questionnaire', { questionnaire }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Questionnaire
     * @param 当前选中数据
     */
    edit(questionnaire) {
      this.childId = questionnaire.id
      this.dialog = true
    },
    /**  停用选中数据  @for Questionnaire
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const questionnaire = this.selectdata(row)
      ...
      if (questionnaire.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Questionnaire
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Questionnaire
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Questionnaire
     * @param 当前选中行
     */
    selectdata(row) {
      const questionnaire = []
  ...
  /** 引用组建 */
  components: {
    'questionnaire-edit': QuestionnaireEdit
  }
}
</script>
