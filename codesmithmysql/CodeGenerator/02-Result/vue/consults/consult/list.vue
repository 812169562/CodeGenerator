...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="consultData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="content" label="咨询内容" width="100"></el-table-column>
         <el-table-column prop="initial" label="父级" width="100"></el-table-column>
         <el-table-column prop="result" label="结果" width="100"></el-table-column>
         <el-table-column prop="evaluate" label="评价" width="100"></el-table-column>
 ...
 <consult-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></consult-edit>
</div>
</template>
<script>
import ConsultEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
     ...
     */
    del(row) {
      const consult = this.selectdata(row)
      if (!consult || consult.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('consult', { consult }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Consult
    ...
    stop(row, isEnable) {
      ...
      if (consult.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Consult
     * @param 当前选中行
     */
