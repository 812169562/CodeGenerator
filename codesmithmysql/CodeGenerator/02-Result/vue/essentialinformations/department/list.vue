...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="departmentData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="isSharePatient" label="是否可共享科室患者" width="100"></el-table-column>
         <el-table-column prop="isExecute" label="是否为执行科室" width="100"></el-table-column>
         <el-table-column prop="isPharmacy" label="是否为药房" width="100"></el-table-column>
         <el-table-column prop="isStorageRoom" label="是否为库房" width="100"></el-table-column>
         <el-table-column prop="registerPrice" label="科室挂号费" width="100"></el-table-column>
 ...
 <department-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></department-edit>
</div>
</template>
<script>
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.departmentData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Department */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Department
     * @param 当前选中数据
     */
    del(row) {
      const department = this.selectdata(row)
      if (!department || department.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('department', { department }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Department
     * @param 当前选中数据
     */
    edit(department) {
      this.childId = department.id
      this.dialog = true
    },
    /**  停用选中数据  @for Department
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const department = this.selectdata(row)
      ...
      if (department.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Department
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Department
     * @param 当前选中行
     */
