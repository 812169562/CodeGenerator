...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="tenantuserrelationData"  @selection-change="handleSelectionChange">
 ...
 <tenantuserrelation-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></tenantuserrelation-edit>
</div>
</template>
<script>
import TenantuserrelationEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
      /** 页面table绑定数据 */
      tenantuserrelationData: [],
      /** 新增编辑页面弹出框  */
      dialog: false,
      /** 分页总数据 初次加载绑定用 */
      total: 15,
  ...
  methods: {
    /**  查询分页信息 @for Tenantuserrelation */
    query() {
      const _this = this
      this.$request.get(
        'tenantuserrelation/page',
        {
        ...
        respose => {
          _this.tenantuserrelationData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Tenantuserrelation */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Tenantuserrelation
     * @param 当前选中数据
     */
    del(row) {
      const tenantuserrelation = this.selectdata(row)
      if (!tenantuserrelation || tenantuserrelation.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('tenantuserrelation', { tenantuserrelation }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Tenantuserrelation
     * @param 当前选中数据
     */
    edit(tenantuserrelation) {
      this.childId = tenantuserrelation.id
      this.dialog = true
    },
    /**  停用选中数据  @for Tenantuserrelation
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const tenantuserrelation = this.selectdata(row)
      ...
      if (tenantuserrelation.find(t => t.isEnable === isEnable)) {
      ...
      this.$request.stop(
        'tenantuserrelation/stop',
        { tenantuserrelation, isEnable },
        respose => {
          this.query()
        },
        null,
    ...
    /**  事件-修改分页大小  @for Tenantuserrelation
     * @param 选中分页大小
     */
    sizeChange(pageSize) {
    ...
    /** 事件-改变分页当前页   @for Tenantuserrelation
     * @param 选中分页大小
     */
    ...
    /** 事件-选中行   @for Tenantuserrelation
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Tenantuserrelation
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Tenantuserrelation
     * @param 当前选中行
     */
    selectdata(row) {
      const tenantuserrelation = []
      if (row) {
        tenantuserrelation.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          tenantuserrelation.push(element)
        })
      }
      return tenantuserrelation
    },
    /** 页面验证
     * @param 类型
  ...
  /** 引用组建 */
  components: {
    'tenantuserrelation-edit': TenantuserrelationEdit
  }
}
</script>
