...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="resourceData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         <el-table-column prop="name" label="资源名称" width="100"></el-table-column>
         <el-table-column prop="stroeId" label="门店编号" width="100"></el-table-column>
         ...
         <el-table-column prop="para" label="参数" width="100"></el-table-column>
         <el-table-column prop="applicationsId" label="" width="100"></el-table-column>
 ...
 <resource-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></resource-edit>
</div>
</template>
<script>
...
export default {
  ...
  methods: {
     ...
     */
    del(row) {
      const resource = this.selectdata(row)
      if (!resource || resource.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('resource', { resource }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Resource
    ...
    stop(row, isEnable) {
      ...
      if (resource.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Resource
     * @param 当前选中行
     */
