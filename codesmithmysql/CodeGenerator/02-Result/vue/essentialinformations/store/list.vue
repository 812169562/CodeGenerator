...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="storeData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="userId" label="店长编号" width="100"></el-table-column>
         ...
         <el-table-column prop="code" label="门店号" width="100"></el-table-column>
 ...
 <store-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></store-edit>
</div>
</template>
<script>
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.storeData = respose.data.result
     ...
     */
    del(row) {
      const store = this.selectdata(row)
      if (!store || store.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('store', { store }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Store
    ...
    stop(row, isEnable) {
      ...
      if (store.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Store
     * @param 当前选中行
     */
