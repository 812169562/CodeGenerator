...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="usersData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="loginCode" label="登陆号" width="100"></el-table-column>
         ...
         <el-table-column prop="wxOpenId" label="微信识别号" width="100"></el-table-column>
         <el-table-column prop="qqOpenId" label="QQ识别号" width="100"></el-table-column>
         <el-table-column prop="type" label="类型" width="100"></el-table-column>
         <el-table-column prop="eMail" label="电子邮件" width="100"></el-table-column>
         <el-table-column prop="isLock" label="是否锁定" width="100"></el-table-column>
 ...
 <users-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></users-edit>
</div>
</template>
<script>
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.usersData = respose.data.result
     ...
     */
    del(row) {
      const users = this.selectdata(row)
      if (!users || users.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('users', { users }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Users
    ...
    stop(row, isEnable) {
      ...
      if (users.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Users
     * @param 当前选中行
     */
