<template>
 <el-dialog :title="title" :visible.sync="dialogVisible" width="55%" height="75%" :close-on-click-modal="false" :before-close="hidden" @open="open"  @close="hidden">
    <el-form :model="form">
         ...
         <mu-text-field label="用户编号" hintText="请输入用户编号" v-model="form.userId"  lab labelFloat/>
         <mu-text-field label="部门编号" hintText="请输入部门编号" v-model="form.departmentId"  lab labelFloat/>
  </el-form>
...
export default {
  ...
  methods: {
    ...
    save() {
      const _this = this
      this.$request.save('userdepartment', this.form, function(respose) {
        if (respose.code === 1) {
          _this.init()
          _this.hidden(true)
        }
    ...
    open() {
      if (!this.id) {
        ...
        this.title = '编辑人员部门信息信息'
        const _this = this
        this.$request.get('userdepartment/' + this.id, {}, (respose) => {
