...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="userdepartmentData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="departmentId" label="部门编号" width="100"></el-table-column>
 ...
 <userdepartment-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></userdepartment-edit>
</div>
</template>
<script>
import UserdepartmentEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.userdepartmentData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Userdepartment */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Userdepartment
     * @param 当前选中数据
     */
    del(row) {
      const userdepartment = this.selectdata(row)
      if (!userdepartment || userdepartment.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('userdepartment', { userdepartment }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Userdepartment
     * @param 当前选中数据
     */
    edit(userdepartment) {
      this.childId = userdepartment.id
      this.dialog = true
    },
    /**  停用选中数据  @for Userdepartment
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const userdepartment = this.selectdata(row)
      ...
      if (userdepartment.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Userdepartment
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Userdepartment
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Userdepartment
     * @param 当前选中行
     */
    selectdata(row) {
      const userdepartment = []
      if (row) {
        userdepartment.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          userdepartment.push(element)
        })
      }
  ...
  /** 引用组建 */
  components: {
    'userdepartment-edit': UserdepartmentEdit
  }
}
</script>
