...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="applicationsData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         <el-table-column prop="name" label="应用程序名称" width="100"></el-table-column>
         <el-table-column prop="code" label="代码" width="100"></el-table-column>
         <el-table-column prop="localHost" label="主机" width="100"></el-table-column>
 ...
 <applications-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></applications-edit>
</div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.applicationsData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Applications */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Applications
     * @param 当前选中数据
     */
    del(row) {
      const applications = this.selectdata(row)
      if (!applications || applications.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('applications', { applications }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Applications
     * @param 当前选中数据
     */
    edit(applications) {
      this.childId = applications.id
      this.dialog = true
    },
    /**  停用选中数据  @for Applications
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const applications = this.selectdata(row)
      ...
      if (applications.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Applications
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Applications
     * @param 当前选中行
     */
  ...
  /** 引用组建 */
  components: {
    'applications-edit': ApplicationsEdit
  }
}
</script>
