...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="diseaseData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="diseaseId" label="四诊数据编号" width="100"></el-table-column>
         <el-table-column prop="diseaseName" label="四诊信息说明" width="100"></el-table-column>
         <el-table-column prop="degree" label="轻重程度(-1代表无此症状,0轻微症状,1严重症状,2非常严重)" width="100"></el-table-column>
 ...
 <disease-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></disease-edit>
</div>
</template>
<script>
import DiseaseEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
     ...
     */
    del(row) {
      const disease = this.selectdata(row)
      if (!disease || disease.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('disease', { disease }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Disease
    ...
    stop(row, isEnable) {
      ...
      if (disease.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Disease
     * @param 当前选中行
     */
