<template>
 <el-dialog :title="title" :visible.sync="dialogVisible" width="55%" height="75%" :close-on-click-modal="false" :before-close="hidden" @open="open"  @close="hidden">
    <el-form :model="form">
         ...
         <mu-text-field label="就诊记录编号" hintText="请输入就诊记录编号" v-model="form.drugId"  lab labelFloat/>
...
export default {
  ...
  methods: {
    ...
    save() {
      const _this = this
      this.$request.save('visitlog', this.form, function(respose) {
        if (respose.code === 1) {
          _this.init()
          _this.hidden(true)
        }
    ...
    open() {
      if (!this.id) {
        ...
        this.title = '编辑就诊记录修改记录表信息'
        const _this = this
        this.$request.get('visitlog/' + this.id, {}, (respose) => {
