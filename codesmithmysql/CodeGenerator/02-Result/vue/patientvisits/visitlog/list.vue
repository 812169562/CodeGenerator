...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="visitlogData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="drugId" label="就诊记录编号" width="100"></el-table-column>
 ...
 <visitlog-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></visitlog-edit>
</div>
</template>
<script>
import VisitlogEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
     ...
     */
    del(row) {
      const visitlog = this.selectdata(row)
      if (!visitlog || visitlog.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('visitlog', { visitlog }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Visitlog
    ...
    stop(row, isEnable) {
      ...
      if (visitlog.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Visitlog
     * @param 当前选中行
     */
