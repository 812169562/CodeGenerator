...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="prescriptionorderData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="stroeName" label="门店名" width="100"></el-table-column>
         <el-table-column prop="code" label="处方编号" width="100"></el-table-column>
         <el-table-column prop="name" label="名称" width="100"></el-table-column>
         <el-table-column prop="prescriptionCode" label="申请单编号" width="100"></el-table-column>
         <el-table-column prop="type" label="申请单类型(1,检验，2检查，3治疗,4.手术.5西药处方,6输液雾化处方,7中药处方,8收费项目,9挂号)" width="100"></el-table-column>
         <el-table-column prop="prescriptionType" label="处方类型(0.普通,1.精一.2.精二.3.中药饮片,4.中药免检.5.中药小包装)" width="100"></el-table-column>
         ...
         <el-table-column prop="expiryTime" label="订单过期时间" width="180"></el-table-column>
         <el-table-column prop="count" label="项目条数" width="100"></el-table-column>
         <el-table-column prop="money" label="项目金额" width="100"></el-table-column>
         <el-table-column prop="reductionMoeny" label="抵扣金额" width="100"></el-table-column>
         ...
         <el-table-column prop="accountingDate" label="核算时间" width="180"></el-table-column>
         <el-table-column prop="remark" label="备注说明" width="100"></el-table-column>
         <el-table-column prop="isModify" label="是否允许修改" width="100"></el-table-column>
 ...
 <prescriptionorder-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></prescriptionorder-edit>
</div>
</template>
<script>
import PrescriptionorderEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    /**  查询分页信息 @for Prescriptionorder */
    query() {
      const _this = this
      this.$request.get(
        'prescriptionorder/page',
        {
        ...
        respose => {
          _this.prescriptionorderData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Prescriptionorder */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Prescriptionorder
     * @param 当前选中数据
     */
    del(row) {
      const prescriptionorder = this.selectdata(row)
      if (!prescriptionorder || prescriptionorder.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('prescriptionorder', { prescriptionorder }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Prescriptionorder
     * @param 当前选中数据
     */
    edit(prescriptionorder) {
      this.childId = prescriptionorder.id
      this.dialog = true
    },
    /**  停用选中数据  @for Prescriptionorder
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const prescriptionorder = this.selectdata(row)
      ...
      if (prescriptionorder.find(t => t.isEnable === isEnable)) {
      ...
      this.$request.stop(
        'prescriptionorder/stop',
        { prescriptionorder, isEnable },
        respose => {
          this.query()
        },
        null,
    ...
    /**  事件-修改分页大小  @for Prescriptionorder
     * @param 选中分页大小
     */
    sizeChange(pageSize) {
    ...
    /** 事件-改变分页当前页   @for Prescriptionorder
     * @param 选中分页大小
     */
    ...
    /** 事件-选中行   @for Prescriptionorder
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Prescriptionorder
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Prescriptionorder
     * @param 当前选中行
     */
    selectdata(row) {
      const prescriptionorder = []
      if (row) {
        prescriptionorder.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          prescriptionorder.push(element)
        })
      }
  ...
  /** 引用组建 */
  components: {
    'prescriptionorder-edit': PrescriptionorderEdit
  }
}
</script>
