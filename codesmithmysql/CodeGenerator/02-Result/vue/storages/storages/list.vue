...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="storagesData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         <el-table-column prop="newName" label="重新生成的名字" width="100"></el-table-column>
         <el-table-column prop="stroeId" label="门店号" width="100"></el-table-column>
         <el-table-column prop="type" label="类型(用于区分压缩后文件比例等）" width="100"></el-table-column>
         <el-table-column prop="catalog" label="文件目录（上传的文件的分类）" width="100"></el-table-column>
         <el-table-column prop="name" label="文件全名" width="100"></el-table-column>
         <el-table-column prop="path" label="文件路径" width="100"></el-table-column>
 ...
 <storages-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></storages-edit>
</div>
</template>
<script>
import StoragesEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
     ...
     */
    del(row) {
      const storages = this.selectdata(row)
      if (!storages || storages.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('storages', { storages }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Storages
    ...
    stop(row, isEnable) {
      ...
      if (storages.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Storages
     * @param 当前选中行
     */
