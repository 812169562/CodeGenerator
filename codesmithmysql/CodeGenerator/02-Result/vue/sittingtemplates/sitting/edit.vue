<template>
 <el-dialog :title="title" :visible.sync="dialogVisible" width="55%" height="75%" :close-on-click-modal="false" :before-close="hidden" @open="open"  @close="hidden">
    <el-form :model="form">
         ...
         <mu-text-field label="使用号数" hintText="请输入使用号数" v-model="form.useCount"  lab labelFloat/>
         <mu-text-field label="限制预约数量" hintText="请输入限制预约数量" v-model="form.appointmentCount"  lab labelFloat/>
         <mu-text-field label="已预约数量" hintText="请输入已预约数量" v-model="form.useAppointmentCount"  lab labelFloat/>
         ...
         <mu-date-picker  label="坐诊日期" hintText="请输入坐诊日期" v-model="form.sittingDate"  lab labelFloat/>
...
export default {
  ...
  methods: {
    ...
    save() {
      const _this = this
      this.$request.save('sitting', this.form, function(respose) {
        if (respose.code === 1) {
          _this.init()
          _this.hidden(true)
        }
    ...
    open() {
      if (!this.id) {
        ...
        this.title = '编辑 医生坐诊表信息'
        const _this = this
        this.$request.get('sitting/' + this.id, {}, (respose) => {
