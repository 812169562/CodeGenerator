...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="accessrecordData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="count" label="领用数量" width="100"></el-table-column>
         <el-table-column prop="unit" label="单位" width="100"></el-table-column>
         <el-table-column prop="accessStatus" label="领用情况" width="100"></el-table-column>
         <el-table-column prop="recipientId" label="创建者" width="100"></el-table-column>
         <el-table-column prop="recipient" label="创建人" width="100"></el-table-column>
 ...
 <accessrecord-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></accessrecord-edit>
</div>
</template>
<script>
import AccessrecordEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.accessrecordData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Accessrecord */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Accessrecord
     * @param 当前选中数据
     */
    del(row) {
      const accessrecord = this.selectdata(row)
      if (!accessrecord || accessrecord.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('accessrecord', { accessrecord }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Accessrecord
     * @param 当前选中数据
     */
    edit(accessrecord) {
      this.childId = accessrecord.id
      this.dialog = true
    },
    /**  停用选中数据  @for Accessrecord
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const accessrecord = this.selectdata(row)
      ...
      if (accessrecord.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Accessrecord
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Accessrecord
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Accessrecord
     * @param 当前选中行
     */
    selectdata(row) {
      const accessrecord = []
      if (row) {
        accessrecord.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          accessrecord.push(element)
        })
      }
      return accessrecord
    },
  ...
  /** 引用组建 */
  components: {
    'accessrecord-edit': AccessrecordEdit
  }
}
</script>
