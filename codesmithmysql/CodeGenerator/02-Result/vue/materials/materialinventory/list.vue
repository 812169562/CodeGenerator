...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="materialinventoryData"  @selection-change="handleSelectionChange">
 ...
 <materialinventory-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></materialinventory-edit>
</div>
</template>
<script>
import MaterialinventoryEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    /**  查询分页信息 @for Materialinventory */
    query() {
      const _this = this
      this.$request.get(
        'materialinventory/page',
        {
        ...
        respose => {
          _this.materialinventoryData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Materialinventory */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Materialinventory
     * @param 当前选中数据
     */
    del(row) {
      const materialinventory = this.selectdata(row)
      if (!materialinventory || materialinventory.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('materialinventory', { materialinventory }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Materialinventory
     * @param 当前选中数据
     */
    edit(materialinventory) {
      this.childId = materialinventory.id
      this.dialog = true
    },
    /**  停用选中数据  @for Materialinventory
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const materialinventory = this.selectdata(row)
      ...
      if (materialinventory.find(t => t.isEnable === isEnable)) {
      ...
      this.$request.stop(
        'materialinventory/stop',
        { materialinventory, isEnable },
        respose => {
          this.query()
        },
        null,
    ...
    /**  事件-修改分页大小  @for Materialinventory
     * @param 选中分页大小
     */
    sizeChange(pageSize) {
    ...
    /** 事件-改变分页当前页   @for Materialinventory
     * @param 选中分页大小
     */
    ...
    /** 事件-选中行   @for Materialinventory
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Materialinventory
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Materialinventory
     * @param 当前选中行
     */
    selectdata(row) {
      const materialinventory = []
      if (row) {
        materialinventory.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          materialinventory.push(element)
        })
      }
  ...
  /** 引用组建 */
  components: {
    'materialinventory-edit': MaterialinventoryEdit
  }
}
</script>
