...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="drugbatchData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         <el-table-column prop="batchNumber" label="批次号（系统按照日期生成）" width="100"></el-table-column>
         ...
         <el-table-column prop="price" label="最小单位价格(自动计算)" width="100"></el-table-column>
         ...
         <el-table-column prop="helflifeDay" label="保质期" width="100"></el-table-column>
 ...
 <drugbatch-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></drugbatch-edit>
</div>
</template>
<script>
import DrugbatchEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.drugbatchData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Drugbatch */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Drugbatch
     * @param 当前选中数据
     */
    del(row) {
      const drugbatch = this.selectdata(row)
      if (!drugbatch || drugbatch.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('drugbatch', { drugbatch }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Drugbatch
     * @param 当前选中数据
     */
    edit(drugbatch) {
      this.childId = drugbatch.id
      this.dialog = true
    },
    /**  停用选中数据  @for Drugbatch
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const drugbatch = this.selectdata(row)
      ...
      if (drugbatch.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Drugbatch
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Drugbatch
     * @param 当前选中行
     */
