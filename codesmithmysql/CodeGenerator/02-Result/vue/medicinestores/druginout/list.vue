...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="druginoutData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="code" label="出入库单号" width="100"></el-table-column>
         <el-table-column prop="type" label="出入库类型" width="100"></el-table-column>
         <el-table-column prop="inOutTypeId" label="出入库类型（来自字典表）" width="100"></el-table-column>
         <el-table-column prop="inOutTypeName" label="出入库类型（来自字典表）" width="100"></el-table-column>
         ...
         <el-table-column prop="applyRemark" label="申请备注" width="100"></el-table-column>
         <el-table-column prop="storeId" label="出入库房" width="100"></el-table-column>
         <el-table-column prop="storeName" label="出入库房" width="100"></el-table-column>
 ...
 <druginout-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></druginout-edit>
</div>
</template>
<script>
import DruginoutEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.druginoutData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Druginout */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Druginout
     * @param 当前选中数据
     */
    del(row) {
      const druginout = this.selectdata(row)
      if (!druginout || druginout.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('druginout', { druginout }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Druginout
     * @param 当前选中数据
     */
    edit(druginout) {
      this.childId = druginout.id
      this.dialog = true
    },
    /**  停用选中数据  @for Druginout
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const druginout = this.selectdata(row)
      ...
      if (druginout.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Druginout
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Druginout
     * @param 当前选中行
     */
