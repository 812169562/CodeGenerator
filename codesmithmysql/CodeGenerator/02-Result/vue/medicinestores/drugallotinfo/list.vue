...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="drugallotinfoData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         <el-table-column prop="drugAllotId" label="调拨编号" width="100"></el-table-column>
         ...
         <el-table-column prop="allotCount" label="调拨入库数量(销售单位为数量)" width="100"></el-table-column>
 ...
 <drugallotinfo-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></drugallotinfo-edit>
</div>
</template>
<script>
import DrugallotinfoEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.drugallotinfoData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Drugallotinfo */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Drugallotinfo
     * @param 当前选中数据
     */
    del(row) {
      const drugallotinfo = this.selectdata(row)
      if (!drugallotinfo || drugallotinfo.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('drugallotinfo', { drugallotinfo }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Drugallotinfo
     * @param 当前选中数据
     */
    edit(drugallotinfo) {
      this.childId = drugallotinfo.id
      this.dialog = true
    },
    /**  停用选中数据  @for Drugallotinfo
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const drugallotinfo = this.selectdata(row)
      ...
      if (drugallotinfo.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Drugallotinfo
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Drugallotinfo
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Drugallotinfo
     * @param 当前选中行
     */
    selectdata(row) {
      const drugallotinfo = []
      if (row) {
        drugallotinfo.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          drugallotinfo.push(element)
        })
      }
      return drugallotinfo
    },
    /** 页面验证
     * @param 类型
  ...
  /** 引用组建 */
  components: {
    'drugallotinfo-edit': DrugallotinfoEdit
  }
}
</script>
