...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="drugupdatelogData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="beforeData" label="更新前数据" width="100"></el-table-column>
         <el-table-column prop="afterData" label="更新后数据" width="100"></el-table-column>
 ...
 <drugupdatelog-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></drugupdatelog-edit>
</div>
</template>
<script>
import DrugupdatelogEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.drugupdatelogData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Drugupdatelog */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Drugupdatelog
     * @param 当前选中数据
     */
    del(row) {
      const drugupdatelog = this.selectdata(row)
      if (!drugupdatelog || drugupdatelog.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('drugupdatelog', { drugupdatelog }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Drugupdatelog
     * @param 当前选中数据
     */
    edit(drugupdatelog) {
      this.childId = drugupdatelog.id
      this.dialog = true
    },
    /**  停用选中数据  @for Drugupdatelog
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const drugupdatelog = this.selectdata(row)
      ...
      if (drugupdatelog.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Drugupdatelog
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Drugupdatelog
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Drugupdatelog
     * @param 当前选中行
     */
    selectdata(row) {
      const drugupdatelog = []
      if (row) {
        drugupdatelog.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          drugupdatelog.push(element)
        })
      }
      return drugupdatelog
    },
    /** 页面验证
     * @param 类型
  ...
  /** 引用组建 */
  components: {
    'drugupdatelog-edit': DrugupdatelogEdit
  }
}
</script>
