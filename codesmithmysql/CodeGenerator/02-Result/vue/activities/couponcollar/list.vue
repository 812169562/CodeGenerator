...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="couponcollarData"  @selection-change="handleSelectionChange">
 ...
 <couponcollar-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></couponcollar-edit>
</div>
</template>
<script>
import CouponcollarEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.couponcollarData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Couponcollar */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Couponcollar
     * @param 当前选中数据
     */
    del(row) {
      const couponcollar = this.selectdata(row)
      if (!couponcollar || couponcollar.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('couponcollar', { couponcollar }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Couponcollar
     * @param 当前选中数据
     */
    edit(couponcollar) {
      this.childId = couponcollar.id
      this.dialog = true
    },
    /**  停用选中数据  @for Couponcollar
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const couponcollar = this.selectdata(row)
      ...
      if (couponcollar.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Couponcollar
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Couponcollar
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Couponcollar
     * @param 当前选中行
     */
    selectdata(row) {
      const couponcollar = []
      if (row) {
        couponcollar.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          couponcollar.push(element)
        })
      }
      return couponcollar
    },
  ...
  /** 引用组建 */
  components: {
    'couponcollar-edit': CouponcollarEdit
  }
}
</script>
