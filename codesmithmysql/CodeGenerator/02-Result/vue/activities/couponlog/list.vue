...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="couponlogData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="userId" label="使用人编号" width="100"></el-table-column>
         <el-table-column prop="count" label="消费条数" width="100"></el-table-column>
 ...
 <couponlog-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></couponlog-edit>
</div>
</template>
<script>
import CouponlogEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.couponlogData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Couponlog */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Couponlog
     * @param 当前选中数据
     */
    del(row) {
      const couponlog = this.selectdata(row)
      if (!couponlog || couponlog.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('couponlog', { couponlog }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Couponlog
     * @param 当前选中数据
     */
    edit(couponlog) {
      this.childId = couponlog.id
      this.dialog = true
    },
    /**  停用选中数据  @for Couponlog
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const couponlog = this.selectdata(row)
      ...
      if (couponlog.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Couponlog
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Couponlog
     * @param 当前选中行
     */
