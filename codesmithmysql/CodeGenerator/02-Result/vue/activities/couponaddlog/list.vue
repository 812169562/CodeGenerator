...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="couponaddlogData"  @selection-change="handleSelectionChange">
 ...
 <el-table-column type="index"  fixed="left" label="序号"    width="55"></el-table-column>
         ...
         <el-table-column prop="couponId" label="优惠卷编号" width="100"></el-table-column>
         <el-table-column prop="count" label="新增条数" width="100"></el-table-column>
 ...
 <couponaddlog-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></couponaddlog-edit>
</div>
</template>
<script>
import CouponaddlogEdit from './edit'
export default {
  /** 页面绑定数据 */
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      this.$request.get(
        ...
        respose => {
          _this.couponaddlogData = respose.data.result
          _this.total = respose.data.totalCount
          _this.current = respose.data.pageIndex
        }
      )
    },
    /**  弹出添加数据页面 @for Couponaddlog */
    add() {
      this.childId = ''
      this.dialog = true
    },
    /**  删除选中数据  @for Couponaddlog
     * @param 当前选中数据
     */
    del(row) {
      const couponaddlog = this.selectdata(row)
      if (!couponaddlog || couponaddlog.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('couponaddlog', { couponaddlog }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Couponaddlog
     * @param 当前选中数据
     */
    edit(couponaddlog) {
      this.childId = couponaddlog.id
      this.dialog = true
    },
    /**  停用选中数据  @for Couponaddlog
     * @param 当前选中数据
     */
    stop(row, isEnable) {
      const couponaddlog = this.selectdata(row)
      ...
      if (couponaddlog.find(t => t.isEnable === isEnable)) {
    ...
    /** 事件-选中行   @for Couponaddlog
     * @param 当前选中行
     */
    handleSelectionChange(val) {
      this.multipleSelection = val
    },
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Couponaddlog
     * @param 当前选中行
     */
    ...
    /** 添加当前选中数据到集合内   @for Couponaddlog
     * @param 当前选中行
     */
    selectdata(row) {
      const couponaddlog = []
      if (row) {
        couponaddlog.push(row)
      } else {
        this.multipleSelection.forEach(element => {
          couponaddlog.push(element)
        })
      }
      return couponaddlog
    },
  ...
  /** 引用组建 */
  components: {
    'couponaddlog-edit': CouponaddlogEdit
  }
}
</script>
