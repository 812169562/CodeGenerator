...
<div>
 ...
 <el-table ref="multipleTable"  column-key="id" tooltip-effect="dark" style="width: 100%" max-height="780" :height="fullHeight" size="small" :data="activityData"  @selection-change="handleSelectionChange">
 ...
 <activity-edit :dialogVisible="dialog" @closeAdd="_addClose" :id="childId"></activity-edit>
</div>
</template>
<script>
...
export default {
  ...
  methods: {
     ...
     */
    del(row) {
      const activity = this.selectdata(row)
      if (!activity || activity.length === 0) {
        this.$message.error('请选择一条数据！')
        return
      }
      this.$request.delete('activity', { activity }, respose => {
        this.query()
      })
    },
    /**  编辑选中数据  @for Activity
    ...
    stop(row, isEnable) {
      ...
      if (activity.find(t => t.isEnable === isEnable)) {
    ...
    /** 回调-子窗口添加完成之后回调关闭刷新当前页面   @for Activity
     * @param 当前选中行
     */
